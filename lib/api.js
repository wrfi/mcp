...
export async function update(shortId, opts = {}) {
  ...
  // so a plain wrfi_update stays safe and never trips the 428.
...
/**
 * Append text to a creation without reading it first (server-serialized, never conflicts by default).
 * @param {string} shortId
 * @param {object} opts - { text, author?, message?, expectedVersion?, idempotencyKey?, appendToken?, editToken?, apiKey? }
 * @returns {Promise<object>} - { ok, version, bytes, offset, len, url }
 */
...
/**
 * Read the last N append entries of a creation (append-aware: author + version per entry).
 * @param {string} shortId
 * @param {object} opts - { n?, json?, password?, editToken?, apiKey? }
 * @returns {Promise<string|object>} - entries text, or { version, count, entries } with json
 */
export async function tail(shortId, opts = {}) {
  const base = getBaseUrl();
  const n = Math.min(Math.max(opts.n || 10, 1), 100);
  const format = opts.json ? "&format=json" : "";
  const res = await httpReq("GET", `${base}/api/raw/${shortId}?tail=${n}${format}`, null, authHeaders(opts));
  return opts.json ? JSON.parse(res.body) : res.body;
}
