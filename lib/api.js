...
// The MCP server must identify as ITSELF: client-specific telemetry,
// compat debugging, and protocol-skew detection all key off this string.
const __pkg = JSON.parse(__rf(__jn(__dn(__fu(import.meta.url)), "..", "package.json"), "utf8"));
const USER_AGENT = `wrfi-mcp/${__pkg.version} wrfi-protocol/1.2`;
/**
...
export async function push(opts) {
  ...
  const body = {
    ...
    // Tri-state, NOT truthiness: `false` is a meaningful value (opt into the
    // speakable id) and must reach the server. Serializing on truthiness
    // silently swallowed every explicit speakable request.
    ...(typeof opts.secure === "boolean" ? { secure: opts.secure } : {}),
